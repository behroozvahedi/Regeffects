    ...
    for line in filein:
        ...
        if species == speciesin and len(promoter) == 10000:
            ...
            for chunk in range(len(tss_chunks)):
                fileout.write('%s-%s\t%s\t%s\t%s\t%s\t%i\n'%(gene, transcript, family, TPM, tss_chunks[chunk], tts_chunks[chunk], chunk))
...
def main():
    ...
    for species in species_set:
        ...
        ProcessSpecies(species, workdir+'data.review.csv', workdir+'caduceus.sequences.%s.tsv'%species, 250, 512)
        #Process sequences for a2z feature extraction
        ProcessSpecies(species, workdir+'data.review.csv', workdir+'a2z.sequences.%s.tsv'%species, 250, 600)
