...
stores, per promoter/terminator pair, ONE ensemble prediction per fold.

The output is a standalone EMPRES predictions file in array_index order
(i.e. per-species blocks, matching the row order of the input .npy files).
All available metadata columns are preserved so each prediction remains linked
to its source row even when transcript identifiers are missing or duplicated.

The output prediction values are kept in log10(1 + TPM) space, matching
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "held-out new-species dataset and save them."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "Directory in which to save the standalone output CSV."
        ),
    )
    parser.add_argument(
        "--phyto_pred_file", type=str, default=None,
        help=(
            "Retained for command-line compatibility but not used, because the "
            "current Hai-derived rows cannot be safely joined to PhytoExpr predictions."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "--save_dir and validate its prediction columns."
        ),
    )
    ...
    if args.phyto_pred_file is not None:
        print(
            "  WARNING: --phyto_pred_file is ignored because the Hai-derived rows "
            "cannot currently be joined safely to PhytoExpr predictions."
        )
    ...
    if join_only:
        ...
        # ------------------------------------------------------------------
        # 5) Build dummy target array required by DNADualDataset for inference.
        # ------------------------------------------------------------------
