...
EMPRES and PhytoExpr is row-aligned. Two output CSVs are written:
  ...
  2. A joined file that merges the 5 EMPRES per-fold ensemble columns onto
     the PhytoExpr predictions file, on (gene, species, transcript), with
...
def main():
    ...
    #    We never compute loss here, but DNADualDataset.__getitem__ returns
    #    a target tensor that the loader yields, so we must supply a finite
    #    float for every sample. We use log10(1 + median_TPM), with NaN
    #    median_TPM replaced by 0.0 (only inside the dataset; the standalone
    #    output still carries the original median_TPM verbatim with NaN preserved).
    # ------------------------------------------------------------------
    ...
    fold_ensemble_preds = {}
    criterion = nn.MSELoss()  # unused for loss; only present to satisfy evaluate_model's signature
    ...
    for fold_num, (val_group, test_group) in enumerate(FOLDS, start=1):
        ...
        # ---- 6a) Standardization stats (per-fold, training-set-derived) ----
        if cfg["standardize"]:
            ...
            # EMPRES 0: identity standardization (OHE pass-through)
            base_C = tss.shape[1]
            base_L = tss.shape[2]
        ...
        # ---- 6b) Build dataloader covering every sample in the new-species set ----
        ...
        # ---- 6c) Evaluate top-X models for this fold ----
        per_model_preds = []
        ...
        pred_stack = np.stack(per_model_preds, axis=0)  # shape (top_x, n_samples)
    ...
    #    Column layout mirrors PhytoExpr's metadata columns + 5 EMPRES columns.
    # ------------------------------------------------------------------
    out_df = pd.DataFrame({
    ...
    # ------------------------------------------------------------------
    # 8) Load PhytoExpr predictions and join strictly onto them
    # ------------------------------------------------------------------
    ...
    if len(phyto) != n_samples:
        raise RuntimeError(
            f"PhytoExpr row count ({len(phyto):,}) does not match "
            f"new-species row count ({n_samples:,})."
        )

    out_df_for_join = out_df[join_keys + empres_cols]
    joined = phyto.merge(
        out_df_for_join, on=join_keys, how="left", validate="one_to_one"
    )

    # Strict mismatch checks (raise on any failure)
    if len(joined) != len(phyto):
        raise RuntimeError(
            f"Joined row count {len(joined):,} differs from PhytoExpr row count {len(phyto):,}."
        )
    n_unmatched = int(joined[empres_cols[0]].isna().sum())
    if n_unmatched > 0:
        raise RuntimeError(
            f"{n_unmatched:,} PhytoExpr rows did not match any EMPRES prediction "
            f"on (gene, species, transcript)."
        )
